...
public class Lab7 {
    ...
    private Comment generateComment(String[] row){
        ...
        int postIde = Integer.parseInt(row[1]);
        String comment = row[5];
        
        Comment c = new Comment(commentId, commentingUserId, postIde, comment, likes);
    ...
    private void runAnalysis(){
        ...
        helper.getAverageLikes();
        helper.postWithMostLikes();
        helper.postWithMostComments();
        helper.commentInacitveUser();
        helper.postInacitveUser();
        helper.userOverall();
    }
}
